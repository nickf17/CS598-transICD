...
def train_epochs(args, model, optimizer, params, dicts):
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch == 0 and not args.test_model:
            # model_dir = MODEL_DIR + "/" + '_'.join([args.model, time.strftime('%b_%d_%H:%M:%S', time.localtime())])
            ...
            model_dir = os.path.join(saved_models, '_'.join([args.model, time.strftime('%b_%d_%H_%M_%S', time.localtime())]))
            os.mkdir(model_dir)
